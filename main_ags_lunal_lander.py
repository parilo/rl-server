...
from random_layer.r_act_discrete import RActDiscrete
...
act_cluster = RActDiscrete (train_loop)
...
async def agent_connection(websocket, path):
    while websocket.open:
        ...
        method = req ['method']
        if method == 'act':
            actions_assoc = assoc.control ([req ['state']])[0]
            # print('--- act {}'.format(actions_assoc))
